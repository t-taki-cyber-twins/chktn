...
function updateSidebarProjectDisplay(project) {
  ...
  sidebarSelected.innerHTML = `
        <div class="sidebar-project-selected-item">
            <div class="sidebar-project-selected-name">${escapeHtml(
              project.name
            )}</div>
...
function initScoutButton() {
  ...
  if (scoutBtn) {
    scoutBtn.addEventListener("click", function () {
      ...
      const engineerName =
        document.querySelector(".form-display-value")?.textContent ||
        "このエンジニア";
      const projectName = selectedProjectData.name;

      if (
        confirm(`「${engineerName}」を「${projectName}」にスカウトしますか？`)
      ) {
...
function initInquiryMessageList() {
  ...
  const threadModalComponent = document.querySelector(
 ...
 */
function loadInquiryMessageContent(
  messageId,
