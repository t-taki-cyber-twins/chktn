...
function initSkillSelection() {
    ...
    // 既存のスキルを取得（初期データ）
    const skills = [];
    const existingSkills = skillList.querySelectorAll('.skill-item');
    existingSkills.forEach((item, index) => {
        const name = item.querySelector('.skill-item-name').textContent;
        const levelText = item.querySelector('.skill-item-level').textContent;
